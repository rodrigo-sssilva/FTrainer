...
--Default input values text generated by the trainer
--defaultInputsText = "Each number represents a virtual key value, that the script\nwill use to assign your custom configuration.\nUse this page as the reference if you want to edit the values."
--defaultInputsReference = "http://www.kbdedit.com/manual/low_level_vk_list.html"
...
--disables all settings that use code injection
...
function HotkeyWarp3()
         ...
         --if inputBreakpointIsActive ~= -1 then sleep(500) end
end
...
function HotkeyWarpCustom3()
         ...
         --workaround to make my life easier with the submarine room, with a new tweak
         --if customFrame == 42 and customCheckFlag == -1 and inputBreakpointIsActive ~= -1 then customCycle = 730 end
...
function SpeedBoost()
         if MiscButtonGeneral() == 0 then return end
         --if readInteger(varPaused) == 0 or readDouble(varSpeedX) == 0 then return end
...
function setDefaultKeys()
         ...
         while i <= hKeyLimitTrue do
               if HotkeyValue[i] == 0 then
                  hFunc[i].setKeys(nil)
                  S.setString(i-1,HotkeyText[i] .. " - None")
               else
                   hFunc[i].setKeys(HotkeyValue[i])
                   S.setString(i-1,HotkeyText[i] .. " - " .. convertKeyComboToString(hFunc[i].getKeys()))
               end
               i = i+1
         end
...
--Change a single key configuration, chosen from the list
...
--Save hotkey values in a file, similar to the default one.
--still supports only 1 key
function saveKeys()
         ...
         i = 1
         while i <= hKeyLimitTrue do
               saveText = saveText .. HotkeyText[i] .. "\n"
               if isempty(hFunc[i].getKeys()) then
                  saveText = saveText .. "0\n"
               else
...
function frameAdvance()
         if inputBreakpointIsActive == 1 then
            ...
            if inputPlaybackLines == 0 then
              ...
              playbackInputRead()
              if inputBreakpointIsActive == 1 then --if playback ended during a frame advance
                 if inputPlaybackCommLines == 0 then
...
function playbackInputs()
         --Perform minor adjustments based on current state
         --[[
         ...
         if inputBreakpointIsActive == 2 then
            --if not isempty(inputPlaybackFile) then inputPlaybackFile:close() end
            debug_continueFromBreakpoint(co_run)
         end
]]
...
function inputFileHeader(file,t)
         ...
         --read and split line
            i0 = file:read("*line")
            i = {}
            if isempty(i0) then break end
            for w in string.gmatch(i0, "[%w%p]+") do
               i[#i+1] = w
            end
            ...
            if i[1] == "character" then
               if i[2] == "lilac" then i[2] = "0" end
               if i[2] == "carol" then i[2] = "2" end
               if i[2] == "carolbike" then i[2] = "3" end
               if i[2] == "milla" then i[2] = "4" end
               i[2] = tonumber(i[2])
               if i[2] ~= math.floor(i[2]) then return 0 end
               if i[2] >= 0 and i[2] <= 4 and i[2] ~= 1 then
                  if i[2] > 1 then i[2] = i[2] - 1 end
                  setProperty(FTrainerMain.CEComboBox1,"ItemIndex",i[2])
                  CEComboBox1Change()
               else return 0 end
            end
            ...
            if i[1] == "difficulty" then
               if i[2] == "hard" then i[2] = "0" end
               if i[2] == "normal" then i[2] = "1" end
               if i[2] == "easy" then i[2] = "2" end
               if i[2] == "casual" then i[2] = "3" end
               i[2] = tonumber(i[2])
               if isempty(i[2]) then return 0 end
               if i[2] ~= math.floor(i[2]) then return 0 end
               if i[2] >= 0 and i[2] <= 3 then
                  writeDouble(varDifficulty,i[2])
               else return 0 end
            end
            ...
            if i[1] == "warp" then
               --custom warps
               if i[2] == "none" or i[2] == "false" then
                  setProperty(FTrainerMain.CEComboBox3,"ItemIndex",0)
               else
               if i[2] == "custom" then
                  --read frame and set it to
                  ...
                  if tonumber(i[4]) ~= 0 then
                     if isempty(i[5]) then return 0 end
                     setProperty(FTrainerMain.BCustomY,"Text",i[5])
                     --if the optional shield values are available
                     if i[6] and i[7] then
                        if isempty(tonumber(i[6])) or isempty(tonumber(i[7])) then return 0 end
                        i[6] = tonumber(i[6])
                        i[7] = tonumber(i[7])
                        if i[6] < 1 or i[6] > 5 then return 0 end
                        if i[7] < 1 or i[7] > 4 then return 0 end
                        setProperty(FTrainerMain.BCustomShield,"ItemIndex",i[6])
                        setProperty(FTrainerMain.BCustomShieldHP,"ItemIndex",i[7]-1)
                     else
                      ...
                      if i[5] and i[6] then
                        if isempty(tonumber(i[5])) or isempty(tonumber(i[6])) then return 0 end
                        i[5] = tonumber(i[6])
                        i[5] = tonumber(i[6])
                        if i[5] < 1 or i[5] > 5 then return 0 end
                        if i[6] < 1 or i[6] > 4 then return 0 end
                        setProperty(FTrainerMain.BCustomShield,"ItemIndex",i[5])
                        setProperty(FTrainerMain.BCustomShieldHP,"ItemIndex",i[6]-1)
                     else
                         setProperty(FTrainerMain.BCustomShield,"ItemIndex",0)
               ...
               --default warps
                   if isempty(tonumber(i[2])) then return 0 end

                   i[2] = tonumber(i[2])
                   if i[2] < 1 or i[2] > 14 then return 0 end
                   if i[2] ~= math.floor(i[2]) then return 0 end
                   setProperty(FTrainerMain.CEComboBox3,"ItemIndex",i[2] + 1)
               end end
            end

            --customshield
            if i[1] == "shield" then
               if isempty(tonumber(i[2])) or isempty(tonumber(i[3])) then return 0 end
               i[2] = tonumber(i[2])
               i[3] = tonumber(i[3])
               if i[2] ~= math.floor(i[2]) then return 0 end
               if i[3] ~= math.floor(i[3]) then return 0 end
               if i[2] < 1 or i[2] > 5 then return 0 end
               if i[3] < 1 or i[3] > 4 then return 0 end
               setProperty(FTrainerMain.BCustomShield,"ItemIndex",i[2])
               setProperty(FTrainerMain.BCustomShieldHP,"ItemIndex",i[3]-1)

               customShield = i[2]
               customShieldHP = i[3] - 1
               UpdateStatusHint()
            end
            ...
            if i[1] == "seed" then
               if i[2] == "false" then
                  setProperty(FTrainerMain.CECheckbox6,"Checked",false)
                   ...
                   if isempty(tonumber(i[2])) then return 0 end
                   if tonumber(i[2]) ~= math.floor(tonumber(i[2])) then return 0 end
                   setProperty(FTrainerMain.BTASSeed,"Text",i[2])
                   setProperty(FTrainerMain.CECheckbox6,"Checked",true)
               end
            end

            --global cycle
            if i[1] == "globalcycle" then
               if isempty(tonumber(i[2])) then return 0 end
               i[2] = tonumber(i[2])
               if i[2] ~= math.floor(i[2]) then return 0 end
               customCycle = i[2]
            ...
            if i[1] == "health" then
               if isempty(tonumber(i[2])) then return 0 end
               i[2] = tonumber(i[2])
               if i[2] < 0 or i[2] > 14 then return 0 end
               customHealth = i[2]
            end
            ...
            if i[1] == "meter" then
               if isempty(tonumber(i[2])) then return 0 end
               i[2] = tonumber(i[2])
               if i[2] < 0 or i[2] > 100 then return 0 end
               customMeter = i[2]
            end

            --input unlock
            if i[1] == "unlock" then
               --if not isempty(i[2]) then return 0 end
               if i[2] == "false" then
                 setProperty(FTrainerMain.CECheckbox2,"Checked",false)
                 customLock = 1
               else if i[2] == "true" or isempty(i[2]) then
            ...
            if i[1] == "speedX" then
               setProperty(FTrainerCustom.CECheckbox1,"Checked",true)
               setProperty(FTrainerCustom.CEEdit1,"Text",i[2])
            end
            if i[1] == "speedY" then
               setProperty(FTrainerCustom.CECheckbox1,"Checked",true)
               setProperty(FTrainerCustom.CEEdit2,"Text",i[2])
            end
            ...
            -- a playback
              ...
              if i[1] == "speed" then
                 if isempty(i[2]) then return 0 end
                 i[2] = tonumber(i[2])
                 if isempty(math.floor(i[2])) then return 0 end
                 setProperty(FTrainerMain.CEComboBox2,"ItemIndex",i[2]-1)
                 CEComboBox2Change()
              end
              ...
              if i[1] == "pause" then
                 ...
                 if i[2] == "false" then
                    setProperty(FTrainerMain.CECheckbox4,"Checked",false)
                     ...
                     if isempty(tonumber(i[2])) then return 0 end
                     if tonumber(i[2]) ~= math.floor(tonumber(i[2])) then return 0 end
                     setProperty(FTrainerMain.TPauseFrameValue,"Text",i[2])
                 end
              end

              --fast forward toggle
              if i[1] == "fastforward" then
                 fftoggle = 1
              end

           --this one is to start TASing right from game boot
              if i[1] == "playfromstart" then
                 if isempty(i[2]) then return 0 end
                 playbackFromStart = true
                 FPVersion = 3
                 createProcess(i[2],nil,true,true)
              end
           end
...
function debugger_onBreakpoint()
         ...
         if inputBreakpointIsActive == -1 or inputBreakpointIsActive == -2 then
            if getProperty(FTrainerMain.CEComboBox3,"ItemIndex") > 0 then
               --hFunc[choosePlaybackWarp(getProperty(FTrainerMain.CEComboBox3,"ItemIndex"))].onHotkey()
            ...
            --if inputBreakpointIsActive == -1 then inputBreakpointIsActive = 2
            --else inputBreakpointIsActive = 5 end
         ...
         if miscTimerLock  then writeDouble(varIgt,600000) end
            ...
            --if miscSpeedScreen then
               --writeInteger(varCrystalDisplay,0)
            --end
         --if miscHud then
         --   writeBytes(varDrawing,{4})
         --end
         --if miscMovement then
               --writeDouble(varJump,-10)
         --end
         ...
         --applies timer stop settings
            if miscTimerStop then
               ...
               else if miscTimerFrame or miscTimerPosX or miscTimerFlag then
                   local x = true
                   if miscTimerFrame then
         ...
         --Applies display commands if no tool options are active
         if inputBreakpointIsActive == 0 then
            ...
            if specialItemBlock ~= 0 then
               if specialItemBlock == 2 then
                  ...
                  writeDouble(varState,30)
                   ...
                   specialItemBlock = 0
                   writeDouble(varBlock,1)load_dialog.Options = "[ofOverwritePrompt]"
               end
            end
            --Activates timer lock when the option has been selected
            if miscExplode then
         ...
         --if recording, write next line to file
         ...
         --forces a pause when the value to pause at is hit
...
function applyPostWarp()
         --RNG Seed stuff
            ...
            --if readInteger(varFrame) == 68 and readDouble(varCheckFlag) == 4 then
            --if customFrame == 68 and customFlag == 4 then
            ...
            --if customFrame == 72 and customFlag < 2 then
...
function playbackInputRead()
         ...
         while true do
            ...
            i0 = inputPlaybackFile:read("*line")
            i = {}
            if isempty(i0) then break end
            for w in string.gmatch(i0, "[%w%p]+") do
         --for w in string.gmatch(inputPlaybackFile:read("*line"), "%w+") do
               i[#i+1] = w
            end

         --forced speed change into inputs
         --remember to add fail conditions later
           if i[1] == "speed" and tonumber(i[2]) then
              --inputPlaybackCommLines = inputPlaybackCommLines + 1
              i[2] = tonumber(i[2])
              setProperty(FTrainerMain.CEComboBox2,"ItemIndex",i[2]-1)
              CEComboBox2Change()
         ...
         --forced pause into inputs
            if i[1] == "pause" and inputBreakpointIsActive == 2 then
               inputPlaybackCommLines = inputPlaybackCommLines + 1
               GamePause()
               fftoggle = 0
            end
         ...
         --commentary stuff
            if i[1] ~= '0' and i[1] ~= '1' and i[1] ~= "." then
               inputPlaybackCommLines = inputPlaybackCommLines + 1
            else
               break
            end
         ...
         inputPlaybackLines = inputPlaybackLines + 1
         lastPlaybackInput = {i[1],i[2],i[3],i[4],i[5],i[6],i[7],i[8]}
         ...
         if fftoggle == 0 then
           ...
           if inputBreakpointIsActive == 3 --[[or inputPlaybackPauseFrame == inputPlaybackLines]] then
              ChangeInputButtons(i[1],i[2],i[3],i[4],i[5],i[6],i[7],i[8])
              if inputPlaybackCommLines == 0 then
                setProperty(FTrainerMain.ToolStatus,"Caption","Paused at frame ".. inputPlaybackLines)
             else
                setProperty(FTrainerMain.ToolStatus,"Caption","Paused at frame ".. inputPlaybackLines .. " (" .. inputPlaybackLines + inputPlaybackCommLines .. ")")
...
function TrainerDragFiles(sender,files)
         --[[if getProperty(FTrainerMain.CEPageControl1,"TabIndex") == 7 then
            playbackFileName = files[1]
            playbackInputs()
         else
         --if getProperty(FTrainerMain.CEPageControl1,"TabIndex") == 4 then
            readKeys(filename,files[1])
         end]]
         f = io.open(files[1])
         local t = f:read("*line")
         if t == "playback" then
...
function CEButton16Click()
         FTrainerCalc.show()
         if inputBreakpointIsActive == 3 then CurrentRNGValue() end
end
...
function ToggleInstantDeath()
         ...
         if miscInstantDeath then
            ...
            nop
             ...
             --do this one if invincibility is still toggled
             if miscInvincibility then
                autoAssemble([[
                "fp.exe"+2A86FF:
                jmp fp.exe+2A8707
...
function ShowSpeedOnScreen()
         ...
         if miscSpeedScreen then
            ...
            --netspeedX
            --calculating speed through x[t] - x[t-1]
            if temp == 3 then
              autoAssemble([[
                newmem6:
                mov eax,[esi+1ba8]
            ...
            --crystal count for improved movement
            if temp == 11 then
              autoAssemble([[
              newmem6:
              //mov eax,[eax+14]
            ...
            miscSpeedScreenDisplay1 = 9999
              miscSpeedScreenDisplay2 = 1
            --writeInteger(varCrystalDisplay,3)
         end
end
...
function ToggleInvincibility()
         ...
         if miscInvincibility then
            ...
            --do not need to do this one if instant death is toggled
...
changeGameSpeedVal = 50

function ChangeGameSpeedTest()
      autoAssemble([[
      alloc(changegamespeed,4)
      registersymbol(changegamespeed)
      ]])
      writeInteger(changegamespeed,changeGameSpeedVal)
      autoAssemble([[
      alloc(newmem,2048)
      label(returnhere)
      label(originalcode)
      ...
      mov [changegamespeed],150
      movd xmm2,[changegamespeed]
      cvtdq2pd xmm2,xmm2
      divsd xmm2,[fp.exe+12beaf8] //=100
      divsd xmm0,xmm2
      originalcode:
      cvttsd2si eax,xmm0
      test eax,eax
...
--Timer Config related functions!!
...
function UpdateTimerStop()
         local x = tonumber(getProperty(FTrainerTimer.CEEdit1,"Text"))
         if not isempty(x) then
            valTimerFrame = x
         end
         x = tonumber(getProperty(FTrainerTimer.CEEdit2,"Text"))
         if not isempty(x) then
            valTimerPosX = x
         end
         x = tonumber(getProperty(FTrainerTimer.CEEdit3,"Text"))
         if not isempty(x) then
            valTimerFlag = x
         end
         miscTimerStop = getProperty(FTrainerTimer.CECheckbox1,"Checked")
...
function UpdateCurrentTimerFrame()
         ...
         valTimerFrame = x
         setProperty(FTrainerTimer.CECheckbox2,"Checked",true)
         setProperty(FTrainerTimer.CEEdit1,"Text",math.floor(x))
         miscTimerFrame = true
end
...
function UpdateCurrentTimerPosX()
         ...
         valTimerPosX = x
         setProperty(FTrainerTimer.CECheckbox3,"Checked",true)
         setProperty(FTrainerTimer.CEEdit2,"Text",Round1(x))
         miscTimerPosX = true
end
...
function UpdateCurrentTimerFlag()
         ...
         valTimerFlag = x
         setProperty(FTrainerTimer.CECheckbox4,"Checked",true)
         setProperty(FTrainerTimer.CEEdit3,"Text",math.floor(x))
         miscTimerFlag = true
end
...
--personal thing
--readKeys(nil,"D:/trainerkeys.txt")
